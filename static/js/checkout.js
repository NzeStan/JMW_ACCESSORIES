document.addEventListener('DOMContentLoaded', function() {
    ...
    'Adamawa': ['Demsa', 'Fufure', 'Ganye', 'Gayuk', 'Gombi', 'Guyuk', 'Hong', 'Jada', 'Lamurde', 
    ...
    'Jigawa': ['Auyo', 'Babura', 'Biriniwa', 'Birnin Kudu', 'Buji', 'Dutse', 'Gagarawa', 'Garki', 
               ...
               'Kazaure', 'Kiri Kasama', 'Maigatari', 'Malam Madori', 'Miga', 'Ringim', 'Roni', 
    ...
    'Kano': ['Ajingi', 'Albasu', 'Bagwai', 'Bebeji', 'Bichi', 'Dala', 'Dambatta', 'Dawakin Kudu', 
             ...
             'Rano', 'Rimin Gado', 'Rogo', 'Shanono', 'Sumaila', 'Takai', 'Tarauni', 
    ...
    'Niger': ['Agaie', 'Agwara', 'Bida', 'Borgu', 'Bosso', 'Chanchaga', 'Edati', 'Gbako', 
              ...
              'Mokwa', 'Muya', 'Paikoro', 'Rafi', 'Shiroro', 'Suleja', 'Tafa', 'Wushishi'],
    ...
    function populateLGAs(stateSelect, lgaSelect, context = '') {
        const state = stateSelect.value;
        console.log(`${context} State selected:`, state);
        
        // Clear and set default option
        lgaSelect.innerHTML = '<option value="">Select LGA</option>';
        
        if (state && LGA_DATA[state]) {
            LGA_DATA[state].forEach(lga => {
                const option = document.createElement('option');
                option.value = lga;
                option.textContent = lga;
                lgaSelect.appendChild(option);
            });
            console.log(`${context}: Added ${LGA_DATA[state].length} LGAs`);
        }
    }

    // Handle NYSC Kit state-LGA relationship
    const nyscState = document.getElementById('nysc-state');
    const nyscLGA = document.getElementById('nysc-lga');

    if (nyscState && nyscLGA) {
        console.log('NYSC Kit handlers initialized');
        nyscState.addEventListener('change', function() {
            console.log('NYSC state changed to:', this.value);
            populateLGAs(this, nyscLGA, 'NYSC');
        });
    }

    // Handle Church Delivery fields
    const pickupToggle = document.getElementById('pickup-toggle');
    const deliveryFields = document.getElementById('delivery-fields');
    
    if (pickupToggle && deliveryFields) {
        // Initial state
        deliveryFields.classList.toggle('hidden', pickupToggle.checked);

        // Handle toggle
        pickupToggle.addEventListener('change', function() {
            const isPickup = this.checked;
            console.log('Church pickup toggled:', isPickup);
            deliveryFields.classList.toggle('hidden', isPickup);
            
            const churchState = document.getElementById('church-state');
            const churchLGA = document.getElementById('church-lga');

            if (!isPickup && churchState && churchLGA) {
                churchState.value = '';
                churchLGA.innerHTML = '<option value="">Select State First</option>';
            }
        });

        // Handle Church state-LGA relationship
        const churchState = document.getElementById('church-state');
        const churchLGA = document.getElementById('church-lga');
        
        if (churchState && churchLGA) {
            churchState.addEventListener('change', function() {
                console.log('Church state changed to:', this.value);
                populateLGAs(this, churchLGA, 'Church');
            });
        }
    }
});